...
@cli.group()
def playlist():
    """Manage video playlists and collections."""
    pass
...
@click.argument('name')
@click.option('--drive', required=True, type=click.Path(exists=True, file_okay=False), help='Path to the external drive')
@click.option('--description', default=None, help='Optional playlist description')
@click.option('--color', default='#3b82f6', help='Playlist color (hex code, default: #3b82f6)')
def create(name: str, drive: str, description: str | None, color: str):
    ...
    with Database(db_path) as db:
        playlist_id = db.create_playlist(name, description, color)
        click.echo(f"Created playlist '{name}' (ID: {playlist_id})")


@playlist.command()
@click.option('--drive', required=True, type=click.Path(exists=True, file_okay=False), help='Path to the external drive')
@click.option('--format', 'output_format', type=click.Choice(['text', 'json']), default='text', help='Output format')
def list(drive: str, output_format: str):
    """List all playlists."""
    drive_path = Path(drive)
    db_path = get_db_path(drive_path)
    ...
    with Database(db_path) as db:
        ...
        if output_format == 'json':
            ...
            click.echo("=" * 60)
            for pl in playlists:
                click.echo(f"  ID: {pl['id']} | {pl['name']}")
                click.echo(f"     Videos: {pl.get('video_count', 0)}")
                if pl.get('description'):
                    click.echo(f"     Description: {pl['description'][:50]}")
                click.echo(f"     Created: {pl.get('created_at', 'Unknown')}")
                click.echo()
...
@click.option('--drive', required=True, type=click.Path(exists=True, file_okay=False), help='Path to the external drive')
@click.option('--format', 'output_format', type=click.Choice(['text', 'json']), default='text', help='Output format')
def show(playlist_id: int, drive: str, output_format: str):
    """Show playlist details and videos."""
    drive_path = Path(drive)
    db_path = get_db_path(drive_path)
    ...
    with Database(db_path) as db:
        ...
        if output_format == 'json':
            ...
            click.echo(f"  Created: {playlist.get('created_at', 'Unknown')}")
            click.echo(f"  Updated: {playlist.get('updated_at', 'Unknown')}")

            if videos:
                click.echo(f"\n  Videos:")
                for i, video in enumerate(videos, 1):
                    click.echo(f"    {i}. [{video['position']}] {video['file_name']}")
                    if video.get('scene_description') and not str(video['scene_description']).startswith('ERROR'):
...
@click.option('--drive', required=True, type=click.Path(exists=True, file_okay=False), help='Path to the external drive')
@click.option('--position', type=int, default=None, help='Position in playlist (defaults to end)')
def add(playlist_id: int, video_id: int, drive: str, position: int | None):
    ...
    with Database(db_path) as db:
        ...
        item_id = db.add_video_to_playlist(playlist_id, video_id, position)
        if item_id:
            click.echo(f"Added '{video['file_name']}' to playlist '{playlist['name']}'")
        else:
            click.echo(f"Video already exists in playlist.", err=True)
            raise SystemExit(1)
...
@click.argument('video_id', type=int)
@click.option('--drive', required=True, type=click.Path(exists=True, file_okay=False), help='Path to the external drive')
def remove(playlist_id: int, video_id: int, drive: str):
    ...
    with Database(db_path) as db:
        ...
        click.echo(f"Removed '{video['file_name']}' from playlist '{playlist['name']}'")
...
def delete(playlist_id: int, drive: str):
    """Delete a playlist."""
    drive_path = Path(drive)
    db_path = get_db_path(drive_path)
    ...
    with Database(db_path) as db:
        ...
        click.echo(f"Deleted playlist '{playlist['name']}' (ID: {playlist_id})")


@playlist.command()
@click.argument('playlist_id', type=int)
@click.option('--drive', required=True, type=click.Path(exists=True, file_okay=False), help='Path to the external drive')
@click.option('--format', type=click.Choice(['fcpxml', 'csv', 'json']), required=True, help='Export format')
@click.option('--output', type=click.Path(), help='Output file path (defaults to playlist name)')
def export(playlist_id: int, drive: str, format: str, output: str | None):
    """Export a playlist to various formats (fcpxml, csv, json)."""
    drive_path = Path(drive)
    db_path = get_db_path(drive_path)
    ...
    with Database(db_path) as db:
        ...
        if not output:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output = f"{safe_name}_{timestamp}.{format}"
        ...
        if format == 'json':
            ...
            with open(output_path, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['Position', 'Video ID', 'File Name', 'File Path', 'Duration (s)', 'Resolution', 'Description'])
                for v in videos:
                    writer.writerow([
                        v['position'],
                        v['id'],
                        ...
                        v.get('resolution', ''),
                        v.get('scene_description', '')[:100] if v.get('scene_description') else ''
                    ])

        elif format == 'fcpxml':
            # FCPXML export - basic structure for Final Cut Pro
            duration_str = "3600s"  # 1 hour default, FCP will adjust
...
<fcpxml version="1.10">
  ...
  <library>
    <event name="{playlist['name']}">
      <project name="{playlist['name']}">
        <sequence format="r1" duration="{duration_str}">
          <spine>
'''
            for i, v in enumerate(videos, 1):
                # Get actual path for FCP
                file_path = v['file_path']
                absolute_path = str(drive_path / file_path)
                # URL-encode the path for FCP
                import urllib.parse
                file_url = 'file://' + urllib.parse.quote(absolute_path)
                xml_content += f'''            <clip name="{v['file_name']}" offset="0s" duration="{v.get('duration_seconds', 0)}s">
              <video>
                <media src="{file_url}"/>
              </video>
            </clip>
'''
        ...
        click.echo(f"Exported playlist '{playlist['name']}' to {output_path}")
